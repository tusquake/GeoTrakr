...
public class GeofenceController {
    ...
    public ResponseEntity<ApiResponse<List<Geofence>>> getAllGeofences() {
        ...
        return ResponseEntity.ok(new ApiResponse<>(true, "Fetched all geofences", geofences));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get geofence by ID")
    public ResponseEntity<ApiResponse<Geofence>> getGeofenceById(@PathVariable Long id) {
        Geofence geofence = geofenceService.getGeofenceById(id);
        return ResponseEntity.ok(new ApiResponse<>(true, "Fetched geofence", geofence));
    }
